...
export function catalogRef(id: string): string {
  return ref("onecount", "catalog", id);
}

/** Parse a catalog ref, returning the bare id, or null if it isn't one. */
export function parseCatalogRef(value: string): string | null {
  const parsed = parseRef(value);
  if (!parsed || parsed.app !== "onecount" || parsed.entity !== "catalog") return null;
  return parsed.id;
}
...
export interface CatalogItemRow {
  ...
  unit_price_ex_gst?: number | null; // PRICE — never projected across the boundary
  [key: string]: unknown;
}
...
export interface CatalogItemFacts {
  ref: string; // onecount:catalog:<id>
  id: string;
  name: string | null;
  brand: string | null;
  category: string | null;
 ...
 */
export function toCatalogItemFacts(row: CatalogItemRow | null | undefined): CatalogItemFacts | null {
